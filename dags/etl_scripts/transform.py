...
def load_transform(source_csv, target_dir):
    df = extract_data(source_csv)
    df = transform_data(df)

    animal = df[["animal_id", "name", "date_of_birth",
                 "animal_type", "breed", "color", "sex"]].drop_duplicates(subset="animal_id")
    
    
    outcome = df[['outcome_type', "outcome_subtype", "age_upon_outcome"]]
    
    time = df[['datetime', "month", "year"]]

    animal.to_parquet(f"{target_dir}_animal_dim.parquet")
    outcome.to_parquet(f"{target_dir}_outcome_dim.parquet")
    time.to_parquet(f"{target_dir}_time_dim.parquet")
